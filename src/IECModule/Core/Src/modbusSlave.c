...
uint16_t Holding_Registers_Database[256] = {0};
uint16_t Input_Registers_Database[256] = {0};
uint8_t Coils_Database[256] = {0};
uint8_t Inputs_Database[256] = {0};

void sendData (uint8_t *data, int size)
{
	HAL_GPIO_WritePin(MCU_DERE_GPIO_Port, MCU_DERE_Pin, GPIO_PIN_SET);
	uint16_t crc = calculateCrc(data, size);
	data[size] = crc&0xFF;   // CRC LOW
	data[size+1] = (crc>>8)&0xFF;  // CRC HIGH
	HAL_UART_Transmit(&huart1, data, size+2, 1000);
	HAL_GPIO_WritePin(MCU_DERE_GPIO_Port, MCU_DERE_Pin, GPIO_PIN_RESET);
}
...
uint8_t writeSingleCoil (void)
{
	...
	if (Coils_Database[startByte] == 0b00000001) HAL_GPIO_WritePin(RELAY_CTRL_GPIO_Port, RELAY_CTRL_Pin, GPIO_PIN_SET);
	else HAL_GPIO_WritePin(RELAY_CTRL_GPIO_Port, RELAY_CTRL_Pin, GPIO_PIN_RESET);
